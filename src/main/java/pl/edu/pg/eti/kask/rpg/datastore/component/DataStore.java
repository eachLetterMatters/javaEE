...
/**
 * For the sake of simplification instead of using real database this example is
 * using a data source object which should
 * be put in servlet context in a single instance. In order to avoid
 * {@link java.util.ConcurrentModificationException}
 * all methods are synchronized. Normally synchronization would be carried on by
 * the database server. Caution, this is
 * very inefficient implementation but can be used to present other mechanisms
 * without obscuration example with ORM
 * usage.
 */
@Log
@ApplicationScoped
...
public class DataStore {
     ...
     * @throws IllegalArgumentException if profession with provided id already
     *                                  exists
     */
     ...
     * @throws IllegalArgumentException if character with provided id already exists
     *                                  or when {@link User} or
     *                                  {@link Profession} with provided uuid does
     *                                  not exist
     */
     ...
     * @throws IllegalArgumentException when {@link User} or {@link Profession} with
     *                                  provided uuid does not exist
     */
    private Character cloneWithRelationships(Character value) {
        ...
        if (entity.getUser() != null) {
            entity.setUser(users.stream()
                    ...
                    .orElseThrow(() -> new IllegalArgumentException(
                            "No user with id \"%s\".".formatted(value.getUser().getId()))));
        }
        ...
        if (entity.getProfession() != null) {
            entity.setProfession(professions.stream()
                    ...
                    .findFirst()
                    .orElseThrow(() -> new IllegalArgumentException(
                            "No profession with id \"%s\".".formatted(value.getProfession().getId()))));
        }

        return entity;
    }

    public synchronized List<Execution> findAllExecutions() {
        return executions.stream()
                .map(cloningUtility::clone)
                .collect(Collectors.toList());
    }

    public synchronized void createExecution(Execution value) throws IllegalArgumentException {
        if (executions.stream().anyMatch(execution -> execution.getId().equals(value.getId()))) {
            throw new IllegalArgumentException("The execution id \"%s\" is not unique".formatted(value.getId()));
        }
        executions.add(cloningUtility.clone(value));
    }

    public synchronized void updateExecution(Execution value) throws IllegalArgumentException {
        if (executions.removeIf(execution -> execution.getId().equals(value.getId()))) {
            executions.add(cloningUtility.clone(value));
        } else {
            throw new IllegalArgumentException("The execution with id \"%s\" does not exist".formatted(value.getId()));
        }
    }

    public synchronized void deleteExecution(UUID id) throws IllegalArgumentException {
        if (!executions.removeIf(execution -> execution.getId().equals(id))) {
            throw new IllegalArgumentException("The execution with id \"%s\" does not exist".formatted(id));
        }
    }
